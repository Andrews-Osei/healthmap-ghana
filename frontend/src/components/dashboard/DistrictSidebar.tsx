...
import {
  Building2, Stethoscope, Pill, Route, Users, MapPin, Sparkles, Clock,
} from "lucide-react";
...
export default function DistrictSidebar() {
  ...
  useEffect(() => {
    if (!sel || sel.data_status === "no_data_yet") {
      setRecs([]); setFcst([]); return;
    }
  ...
  const isPending = sel.data_status === "no_data_yet";
    ...
    <AnimatePresence mode="wait">
      <motion.div
        ...
        <div className="glass p-5">
          ...
          {isPending ? (
            <div className="mt-4 flex items-start gap-3 p-3 rounded-lg
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1 leading-relaxed">
                  No health facilities have been mapped to this district in
                  OpenStreetMap yet. It will be auto-scored on the next
                  pipeline refresh that picks up new OSM data — likely a
                  high-vulnerability district given the absence of mapped
                  facilities.
                </p>
              </div>
            </div>
            ...
            <div className="mt-4 flex items-center gap-4">
              <VulnerabilityGauge value={sel.CVI_0_100 ?? 0}/>
              <div>
                ...
                <div className="text-xs text-slate-400 mt-2">
                  Rank #{sel.rank} of scored districts
                </div>
              </div>
            </div>
          )}
        ...
        {/* Statistics — only meaningful if we have data */}
        {!isPending && (
        ...
        {!isPending && <RecommendationsCard recs={recs}/>}
        {!isPending && <ForecastChart rows={fcst}/>}
      </motion.div>
    </AnimatePresence>
  );
}
